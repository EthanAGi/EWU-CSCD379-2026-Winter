    ...
    <template v-else>
      ...
      <section class="card">
        ...
        <div v-else class="muted">
          ...
          <span v-if="isMortician" class="muted">
            If you are a Mortician, you should be auto-assigned (server-side).
          </span>
        </div>
      </section>
      ...
      <section v-if="viewMode === 'create'" class="card">
        ...
        <form class="formGrid" @submit.prevent="createCase">
          ...
          <div class="formActions">
            ...
            <button class="btn" type="button" @click="viewMode = 'open'" :disabled="creating">
              Back to list
            </button>
          </div>
        </form>
      ...
      <main v-if="viewMode === 'open'" class="grid">
        ...
        <section class="card">
          <div class="listHeader">
            ...
            <div class="chips">
              <button class="chip" :class="{ active: listMode === 'mine' }" @click="listMode = 'mine'">
                My Cases
              </button>
          ...
          <ul class="caseList" v-if="filteredCases.length">
            ...
            >
              ...
              <div class="rowBottom">
                <span class="muted">Assigned: {{ c.assignedMortician?.displayName ?? "—" }}</span>
              </div>

              <div v-if="isAdmin && !c.assignedMortician" class="muted small" style="margin-top: 6px">
                Needs assignment
              </div>
            </li>
          </ul>
          ...
          <div v-else>
            <div class="detailHeader">
              <div>
                ...
                <div class="detailMeta">
                  <span class="badge" :class="caseStatusClass(selectedCase.status)">
                    {{ selectedCase.status }}
                  </span>
                ...
                <div class="detailMeta">
                  <span class="muted">Assigned: {{ selectedCase.assignedMortician?.displayName ?? "—" }}</span>
                </div>
              </div>
              ...
              <div class="detailActions">
                ...
                <!-- Admin assignment UI: assign/reassign -->
                <div v-if="isAdmin" class="assignBox">
                  ...
                  <div class="assignActions">
                    <button
                      class="btn primary"
                      @click="assignMorticianToSelected(selectedMorticianId)"
                      :disabled="assigning || !selectedMorticianId"
                      title="Assign or reassign selected mortician"
                    >
              ...
              <li v-for="t in tasks" :key="t.id" class="taskRow">
                <div class="taskLeft">
                  <div class="taskName">
                    ...
                    <div class="muted" v-if="t.workflowStepTemplate.description">
                      {{ t.workflowStepTemplate.description }}
                    </div>
                    <div class="muted">
                      Started: {{ formatDateTime(t.startedAt) }} • Completed: {{ formatDateTime(t.completedAt) }}
                    </div>
                  </div>
                </div>
            ...
            <form class="noteCreate" @submit.prevent="addNote">
              ...
              <button
                class="btn primary"
                type="submit"
                :disabled="addingNote || !newNoteText || selectedCase.status === 'Completed'"
              >
 ...
 * API base (optional)
 * nuxt.config.ts:
 * runtimeConfig: { public: { apiBase: "https://your-api-host" } }
 */
...
function updateListAssignedMortician(caseNumber: string, m: MorticianDto | null): void {
  ...
  // ✅ FIX: TS “possibly undefined” guard
...
async function assignMorticianToSelected(userId: string): Promise<void> {
  ...
  try {
    ...
    // Optimistic UI update (and TS-safe)
    if (resp?.assignedMortician) {
      const m: MorticianDto = {
        id: resp.assignedMortician.id,
        email: resp.assignedMortician.email ?? null,
        displayName: resp.assignedMortician.displayName ?? resp.assignedMortician.email ?? resp.assignedMortician.id,
      }

      // ✅ sc is a local non-null reference
